...
class Player:
    ...
    def update_elo(self,elo_ot,elo_partner,winner,pre_calc=False,odds=.5):
        """
        ...
        elo_partner: float
            The ELO of this player's partner in a given match
        k1 : int
...
class ELO_Model:
    ...
    def record_game(self,player1,player2,player3,player4,winner,index):
        ...
        if (winner == 1) :
            ...
            self.temp_games.at[index,"T1P1 Change"]=player1.update_elo(elo2,player2.elo,pre_calc=self.avg_team,odds=cutoff,winner=True)
            self.temp_games.at[index,"T1P2 Change"]=player2.update_elo(elo2,player1.elo,pre_calc=self.avg_team,odds=cutoff,winner=True)
            self.temp_games.at[index,"T2P1 Change"]=player3.update_elo(elo1,player4.elo,pre_calc=self.avg_team,odds=cutoff,winner=False)
            self.temp_games.at[index,"T2P2 Change"]=player4.update_elo(elo1,player3.elo,pre_calc=self.avg_team,odds=cutoff,winner=False)
        ...
        # Case if team 2 wins
            ...
            # Updating the Elo Ratings
            self.temp_games.at[index,"T1P1 Change"]=player1.update_elo(elo2,player2.elo,pre_calc=self.avg_team,odds=cutoff,winner=False)
            self.temp_games.at[index,"T1P2 Change"]=player2.update_elo(elo2,player1.elo,pre_calc=self.avg_team,odds=cutoff,winner=False)
            self.temp_games.at[index,"T2P1 Change"]=player3.update_elo(elo1,player4.elo,pre_calc=self.avg_team,odds=cutoff,winner=True)
            self.temp_games.at[index,"T2P2 Change"]=player4.update_elo(elo1,player3.elo,pre_calc=self.avg_team,odds=cutoff,winner=True)
